...
- Card per provider: name, type dropdown, base URL, **API key** — write-only: paste a plain key once ("stored server-side, never displayed") *or* an env-var name; pasted values are never shown back (GET returns only "set / not set"), and leaving the field blank on edit keeps the existing credential. *(Plain-key support implemented in `acb5ee2`; env-var name remains supported.)* Clearing semantics are asymmetric by design: **blank env-var field = remove the reference; blank pasted-key field = keep the existing value** — the staged body must reflect exactly this (no stale `api_key_env` riding along). *(Fix in `6d8b7d6`.)* **Removing a credential is an explicit action**: the edit form carries a "remove stored key" checkbox (distinct from leave-blank-keeps) that stages the provider with neither `api_key` nor `api_key_env` — confirmed before staging, since it can't be undone without re-pasting; "no credential" is a legal state (keyless providers like Ollama). Credential precedence on the server: pasted `api_key` wins over `api_key_env`; removing the credential on a type that requires auth still validates and fails at request time (per the valid-but-fails ruling).
...
- **Legal-value expressibility (positive path, all forms)**: every input control must be able to express at least one complete, valid value (e.g. an alias target field must accept full `provider:model` strings — a selector offering only `provider:` prefixes makes the legal value unreachable). Constrained widgets (dropdowns, autocompletes) are fine only when they cover a legal value end-to-end; otherwise use free text with suggestions.
